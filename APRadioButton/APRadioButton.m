...
@interface APRadioButton()
{
    NSMutableArray* _sharedLinks;
}
@end

@implementation APRadioButton
...
{
    if(!_sharedLinks) {
        for(APRadioButton* rb in buttons) {
            if(rb->_sharedLinks) {
    ...
    BOOL (^btnExistsInList)(NSArray*, APRadioButton*) = ^(NSArray* list, APRadioButton* rb){
        ...
        if(rb->_sharedLinks!=_sharedLinks) {
            if(!rb->_sharedLinks) {
                ...
                for(NSValue* v in rb->_sharedLinks) {
                    APRadioButton* vrb = [v nonretainedObjectValue];
...
-(APRadioButton*) selectedButton
{
    if([self isSelected]) {
        return self;
    } else {
        for(NSValue* v in _sharedLinks) {
            APRadioButton* rb = [v nonretainedObjectValue];
            if([rb isSelected]) {
                return rb;
            }
        }
