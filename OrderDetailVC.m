...
@interface OrderDetailVC()<UIGestureRecognizerDelegate>
{
    UITextField *txt_qty;
}
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(dict)
            {
                ...
                if ([object_OM.msg isEqualToString:@"records not found"]) {
                    ...
                    NSLog(@"ORDER DETAILS: %@", arr_orderDetails);
...
-(void)receiveAllItems{
    ...
    //arr_orderDetails[indexPath.section][@"serialized"]
    for (NSMutableDictionary *item in arr_orderDetails) {
        if ([[item valueForKey:@"serialized"] isEqualToString:@"0"]){
            [self textFieldDidEndEditing:universalTextField];
            break;
        }
    }
     ...
     */
        ...
        [[AppDelegate sharedInstance] showCustomLoader:self];
        dispatch_async(myQueue, ^{
            NSDictionary *dict =[object_OV receiveOrderRT_ID:RT_ID transfer_id:transfer_id order_id:self.order_ID from_id:from_Id json_request:json];
            dispatch_async(dispatch_get_main_queue(), ^{
                [[AppDelegate sharedInstance] removeCustomLoader];
...
-(NSString*)jsonString{
    ...
    for (NSDictionary *saveDict in arr_orderDetails) {
        ...
        NSString *item_id           = [saveDict valueForKey:@"item_id"];
        NSString *serialized        = [saveDict valueForKey:@"serialized"];
        NSString *quantityReceived  = [saveDict valueForKey:@"quantity"]; //@"quantity"
        NSString *quantityActual    = [saveDict valueForKey:@"quantityActual"];
        NSString *check = [NSString stringWithFormat:@"%@", [saveDict valueForKey:@"quantityActual"]];
        NSString *quantityBackorder = [saveDict valueForKey:@"back_order_qty"];
        ...
        NSString *item = [NSString stringWithFormat:@"{\"item_id\": \"%@\",\"serialized\": %@, \"quantityReceived\": %@, \"quantityActual\": %@, \"serial_list\": %@, \"quantityBackorder\": %@}", item_id, serialized, quantityReceived, quantityActual, serial_list, quantityBackorder];
...
{
    static NSString *CellIdentifier = @"EditOrderDetail";
    ...
    else
    {
        ...
        if ([arr_orderDetails[indexPath.section][@"serialized"] isEqualToString:@"0"]) {
            cell.lbled_serialNum.text   = @"Unserialized";
        }
        else{
            cell.lbled_serialNum.text    = @"No Serial";
            if (self.arr_serialNumbers.count > 0) {
                cell.lbled_serialNum.text = [[self.arr_serialNumbers objectAtIndex:indexPath.row] valueForKey:@"serial"];
...
//    txt_qtySent.backgroundColor     = [UIColor colorWithRed:28.0/255.0 green:163.0/255.0 blue:217.0/255.0 alpha:1.0];
    ...
    txt_qty            = [[UITextField alloc] initWithFrame:CGRectMake(536, 0, 113+106, 33)];
    txt_qty.keyboardType            = UIKeyboardTypeNumberPad;
    txt_qty.text                    = [NSString stringWithFormat:@"%@", orderDict[@"quantityActual"]];
    txt_qty.tag                     = section;
    txt_qty.delegate                = self;
    txt_qty.userInteractionEnabled  = ![orderDict[@"serialized"] boolValue];
    if (!txt_qty.userInteractionEnabled) txt_qty.alpha = 0.5;
...
//    txt_qty.backgroundColor         = [UIColor colorWithRed:28.0/255.0 green:163.0/255.0 blue:217.0/255.0 alpha:1.0];
    ...
    */
    if ([orderDict[@"serialized"] boolValue]) {
        ...
        //[ddict setObject:txt_qty.text forKey:@"quantityActual"];
    ...
    UILabel *headerItemType           = [[UILabel alloc] initWithFrame:CGRectMake(327, 0, 80, 33)]; //w: 113
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if (![textField.text isEqualToString:@""]) {
        ...
        [ddict setObject:@"" forKey:@"quantityActual"];
        [arr_orderDetails replaceObjectAtIndex:universalTextField.tag withObject:ddict];
    }
