...
#define DEFAULT_TAB_BAR_HEIGHT 200
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex
{
    if (selectedIndex != _selectedIndex && selectedIndex < [self.viewControllers count])
    {
        ...
        selectedViewController.view.frame = CGRectMake(self.tabBarWidth,
                                                       90,
                                                       self.view.frame.size.width-self.tabBarWidth,
...
- (void)loadView
{
    ...
    // create tab bar
    self.tabBar.frame = CGRectMake(0, 90, self.tabBarWidth, layoutContainerView.bounds.size.height);
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(tabBarController:shouldSelectViewController:)]) {
        ...
        UIViewController *newController = [[[self.viewControllers objectAtIndex:indexPath.row] viewControllers] objectAtIndex:0];
